...
testTokenizerSuccess = 
  tokenizeIOMain lexer "" $ prepTokens $ tokens "1 + 2 - 3 * 4 ^ 5 ! == 1 \n -- blah blah \n {- ml {{- ml --}-} -- blah\nb" 
...
testParserSuccess = do
  parseIOMain cpExp "" ∘ stream *$ prepTokens ^$ tokenizeIO lexer "" $ prepTokens $ tokens "(- 1) + - 2 + 3 * 4 ^ 5 ^ 6 !"

testParserFailure1 ∷ IO ()
testParserFailure1 = parseIOMain cpExp "" ∘ stream *$ prepTokens ^$ tokenizeIO lexer "" $ prepTokens $ tokens "((9 == ((- 1))) + 2 + 3 * 4 ^ 5 ^ 6 !))"

testParserFailure2 ∷ IO ()
testParserFailure2 = parseIOMain cpExp "" ∘ stream *$ prepTokens ^$ tokenizeIO lexer "" $ prepTokens $ tokens "(((((- 1))) + 2 + 3 * 4 ^ 5 ^ ! == 0))"
