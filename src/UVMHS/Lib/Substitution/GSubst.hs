...
-- conceptually) to a SubstNameless. In order to perform substitutions on
-- SubstNameless, you
...
--
-- Put another way, you can think of `DVar` substitutions using `SubstNameless` and `NVar`
-- substitutions using `𝕎 ⇰ SubstNameless`. When you keep around the GVar and MVar
...
data GSubst s₁ s₂ e = GSubst
  ...
  , gsubstSubst ∷ s₂ ⇰ SubstNameless s₂ e
  }
...
𝓈shiftG 𝑠 (GSubst esᴳ 𝓈s) =
  let esᴳ' = map (introSubstElem 𝑠) esᴳ
      𝓈s' = kmapOn 𝓈s $ \ s (SubstNameless ρ es ι) →
        let ρ'  = ρ + ifNone 0 (𝑠 ⋕? s)
            es' = mapOn es $ introSSubstElem s 𝑠
        in SubstNameless ρ' es' ι
  in GSubst esᴳ' 𝓈s'
...
𝓈introG 𝑠 = GSubst null $ mapOn 𝑠 $ SubstNameless 0 null ∘ intΩ64
...
appendGSubst esubst 𝓈̂₂ 𝓈̂₁ =
  let GSubst esᴳ₁ 𝓈s₁ = 𝓈̂₁
      ...
      𝓈s₁' = kmapOn 𝓈s₁ $ \ s (SubstNameless ρ̇₁ es₁ ι₁) → SubstNameless ρ̇₁ (mapOn es₁ $ ℯsub s 𝓈̂₂) ι₁
      esᴳ = esᴳ₁' ⩌ esᴳ₂
      -- esᴹ = esᴹ₁' ⩌ esᴹ₂
      𝓈s = dunionByOn 𝓈s₂ 𝓈s₁' $ \ 𝓈₂@(SubstNameless ρ̇₂ es₂ ι₂) 𝓈₁@(SubstNameless ρ̇₁ es₁ ι₁) →
        if
        | isNullSubstNameless 𝓈₁ → 𝓈₂
        | isNullSubstNameless 𝓈₂ → 𝓈₁
        | otherwise →
