module UVMHS.Lib.Substitution.USubsty where
...
𝓈sdintro = Subst ∘ introSubstSpaced ∘ assoc ∘ map (mapFst $ flip (:*) None) ∘ iter

𝓈snintro ∷ (Ord s) ⇒ s ⇰ 𝕎 ⇰ ℕ64 → Subst s e
𝓈snintro 𝑠 = Subst $ introSubstSpaced $ assoc $ do
  s :* xns ← iter 𝑠
  x :* n ← iter xns
  return $ s :* Some x :* n
...
𝓈sdbinds = Subst ∘ sbindsSubstSpaced ∘ assoc ∘ map (mapFst $ flip (:*) None) ∘ iter
...
𝓈snbinds 𝑠 = Subst $ sbindsSubstSpaced $ assoc $ do
  s :* xess ← iter 𝑠
  x :* es ← iter xess
  return $ s :* Some x :* es
...
𝓈sgbinds sxes = Subst $ ubindsSubstSpaced $ assoc $ do
  s :* xes ← iter sxes
  x :* e ← iter xes
  return $ s :* x :* e
...
substyVar xO s 𝓋 n = do
  ...
  case γ of
    FVsSubstEnv 𝒶 → do
      ...
      return $ 𝓋 n
    SubSubstEnv 𝒶 → do
      let 𝓈s = substSpacedScoped $ unSubst $ substActionSubst 𝒶
      case 𝓈s ⋕? (s :* xO) of
        None → return $ 𝓋 n
