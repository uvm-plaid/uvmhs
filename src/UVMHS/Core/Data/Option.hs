...
instance Functor 𝑂 where map = mmap
instance Return 𝑂 where return = Some
instance Bind 𝑂 where xO ≫= k = case xO of {None → None;Some x → k x}
instance Monad 𝑂
instance FunctorM 𝑂 where 
  mapM f = \case
    None → return None
    Some x → Some ^$ f x
instance (Null a) ⇒ Null (𝑂 a) where null = Some null
